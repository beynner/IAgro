-- SANKHYA.TRG_INC_UPD_TGFRAT
CREATE OR REPLACE TRIGGER SANKHYA.TRG_INC_UPD_TGFRAT
"SANKHYA".TRG_INC_UPD_TGFRAT 
BEFORE INSERT OR UPDATE ON  TGFRAT FOR EACH ROW
...
IF STP_GET_ATUALIZANDO THEN
  ...
  ELSIF ((:NEW.ORIGEM = 'F') OR (:NEW.ORIGEM = 'R')) THEN
    ...
    EXCEPTION WHEN NO_DATA_FOUND THEN 
        BLOQALTRAFINCTB := 'S'; --O DEFAULT EH LIGADO
    END;

    IF (:NEW.NUFIN <> TGFFIN_PKG.V_NUFINRECOMPANT) AND (NVL(BLOQALTRAFINCTB, 'S') = 'S') THEN  --O DEFAULT DE BLOQALTRAFINCTB EH‰ LIGADO
      SELECT COUNT(1) INTO P_COUNT
      FROM TCBINT C
      WHERE C.NUNICO = :NEW.NUFIN
      AND C.ORIGEM IN ('F','B');
