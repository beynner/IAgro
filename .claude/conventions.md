...
- **Swipe-to-edit + delete** (ou view, armar, etc) nos cards — arrasta esquerda revela botões escondidos atrás:
  ...
  - **Swipe bidirecional (Mai/2026 — 2026-05-28)**: card pode ter swipe-esquerda E swipe-direita simultaneamente revelando ações diferentes. Implementação canônica em `rastreio_mobile.js` (cards de lote): esquerda revela armar+olho (88px, padrão antigo); direita revela avaria de ajuste (60px, novo). Constantes JS: `SWIPE_REVEAL_AVARIA = 60` · `SWIPE_TRIGGER_AVARIA_PX = 30` (50% do reveal). **1 swipe aberto por vez em qualquer direção** — abrir em uma fecha qualquer outro (incluindo na outra direção). Estado controlado por dois atributos: `data-swipe-open="1"` (esquerda) ou `data-swipe-right="1"` (direita); ambos zerados em `fecharTodosSwipesX()`, `setActiveScreen` e `openSheet`. Resistência elástica em ambas direções no overswipe. Detalhes em [`modules/rastreio.md`](modules/rastreio.md) → "Avaria de Ajuste (TOP 33)"
