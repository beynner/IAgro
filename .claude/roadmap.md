...
| **Vincular lote em nota faturada (TGFVAR)** | Pesquisa concluída em 2026-05-09: vínculo nativo Sankhya está em **TGFVAR** (não TGFCAB), 185k pares pedido↔nota mapeados. Risco fiscal zero (NFe não carrega lote em hortifrúti). Plano completo documentado em `.claude/modules/rastreio.md` → "Fase 2 planejada" | Implementar `atribuir_lote_pedido_finalizado` em `oracle_conn.py` + toggle "Incluir finalizados" no frontend + audit em `RastreioAudit` | Aprovar plano detalhado e executar (~3-4h) |
