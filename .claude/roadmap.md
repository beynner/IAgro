...
| **Vincular lote em nota faturada (TGFVAR)** | ✅ Implementado em 2026-05-11 (commit pendente). Funções `atribuir_lote_pedido_finalizado` e `desvincular_lote_pedido_finalizado` em `oracle_conn.py` + 2 endpoints novos + toggle `Incluir finalizados` no Rastreio + badge laranja `FATURADO` em cards TOP 35/37. Propagação via TGFVAR pro par pedido↔nota. 9 testes novos passam (183 total no projeto). Detalhes completos em `.claude/modules/rastreio.md` → "Fase 2 IMPLEMENTADA" | Limitação consciente: split em pedido faturado não suportado (desvincular + reatribuir) | Em testes em produção |
| **`AD_NUMPEDIDOORIG` em pedidos de venda** | Default automático na `inserir_cabecalho_nota_banco` (linha 597). Páginas Vendas e Importação por e-mail herdam naturalmente | Em produção, 298/298 pedidos TOP 34 recentes com NULL — pedidos vêm do Sankhya direto, IAgro tem pouco uso na criação ainda | Acompanhar adoção; migração retroativa via TGFVAR fica como pendência futura |
